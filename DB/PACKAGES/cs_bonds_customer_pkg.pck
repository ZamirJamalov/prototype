create or replace package cs_bonds_customer_pkg is

  -- Author  : USER
  -- Created : 9/23/2017 1:09:23 PM
  -- Purpose : 
...
end cs_bonds_customer_pkg;
/
create or replace package body cs_bonds_customer_pkg is
...
BEGIN
    ...
    json_kernel.append_as_sql(p_json_part => '{"row@rownum":["@id","@customer_code","@credit_no","@initial_amount","@remaining_debt","@annuity","@closed"]}',
                            p_sql       => 'select rownum,a.id as id,a.customer_code as customer_code,a.credit_no as credit_no,a.initial_amount as initial_amount,a.remaining_debt as remaining_debt,a.annuity as annuity, a.closed as closed
                             from (select id,customer_code,credit_no,initial_amount,remaining_debt,annuity, case closed when ''Y'' then ''Bəli'' else ''Xeyr'' end as closed from scoring.cs_bonds_customer order by '||v_idx||' '||v_sort_order||' ) a');  
...
BEGIN
  RETURN cs_bonds_customer_seq.nextval;
END setid;   

FUNCTION add RETURN CLOB IS
BEGIN
  INSERT INTO cs_bonds_customer(id,
                                customer_code,
                                credit_no,
                                initial_amount,
                                remaining_debt,
...
FUNCTION upd RETURN CLOB IS 
  v_id cs_bonds_customer.id%TYPE DEFAULT api_component.getvalue('id');
BEGIN
  UPDATE cs_bonds_customer a SET a.customer_code=api_component.getvalue('customer_code'),
...
FUNCTION del RETURN CLOB IS
  v_id cs_bonds_customer.id%TYPE DEFAULT api_component.getvalue('id');
BEGIN
  DELETE FROM cs_bonds_customer WHERE id=v_id;
  COMMIT;
  RETURN uiresp('message','OK');
 EXCEPTION
   WHEN OTHERS THEN 
